        ...
        <main className="p-6 sm:p-8 space-y-8 max-w-6xl w-full mx-auto">
          ...
          <section className="rounded-2xl bg-gradient-to-r from-indigo-50 via-white to-slate-50 dark:from-slate-900 dark:via-slate-900 dark:to-indigo-950/40 border border-indigo-100 dark:border-indigo-900/40 p-6 sm:p-7 shadow-sm flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            <div className="flex items-start sm:items-center gap-4">
              <div className="w-14 h-14 rounded-2xl bg-indigo-600 text-white flex items-center justify-center text-2xl shadow-lg shadow-indigo-600/30 shrink-0">
                🧹
              </div>
              <div>
                    ...
                    <div>
                      ...
                      <div className="flex items-center justify-between mb-3">
                        ...
                        {esMultado ? (
                          ...
                          <span className="px-2.5 py-0.5 rounded-full bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 text-[11px] font-semibold border border-emerald-200">
                            ✓ Cumplido
                          </span>
                        ) : (
          ...
          {/* 3. FONDO ESPECIAL DE MULTAS Y COMPENSACIONES */}
          {multasHistorial.length > 0 && (
            <section className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm">
              <div className="flex items-center gap-2 text-rose-600 dark:text-rose-400 text-xs font-bold uppercase tracking-wider mb-2">
                <span>⚖️</span>
                <span>Fondo Especial de Limpieza y Sanciones</span>
              </div>
              <h3 className="text-base font-bold text-slate-900 dark:text-white mb-4">
                Registro de Multas y Créditos Compensatorios
              </h3>
                ...
                <table className="w-full text-left text-xs">
                  <thead className="bg-slate-50 dark:bg-slate-950 text-slate-400 uppercase border-b border-slate-200 dark:border-slate-800">
                    <tr>
                      ...
                      <th className="px-4 py-2.5">UF Infractora (Débito)</th>
                      <th className="px-4 py-2.5">UF Sustituta (Crédito)</th>
                      <th className="px-4 py-2.5">Monto Imputado</th>
                    ...
                    {multasHistorial.map((m) => (
                      <tr key={m.id} className="hover:bg-slate-50 dark:hover:bg-slate-950/50">
                        ...
                        <td className="px-4 py-3 font-bold text-rose-600">
                          UF 0{m.infractora} (Débito Expensa)
                        </td>
                        <td className="px-4 py-3 font-bold text-emerald-600">
                          UF 0{m.sustituta} (Crédito a Favor)
                        </td>
                        <td className="px-4 py-3 font-bold text-slate-900 dark:text-white">
                          ${m.monto.toLocaleString("es-AR")}
                        </td>
                        <td className="px-4 py-3">
                          <span className="px-2 py-0.5 rounded-full bg-indigo-50 dark:bg-indigo-950 text-indigo-600 dark:text-indigo-400 font-semibold border border-indigo-200 dark:border-indigo-800">
                            Imputada en BD
                          </span>
                        </td>
                      </tr>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 backdrop-blur-sm p-4 animate-fade-in">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl max-w-lg w-full p-6 sm:p-7 shadow-2xl relative">
            ...
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 rounded-xl bg-rose-50 dark:bg-rose-950 text-rose-600 dark:text-rose-400 flex items-center justify-center text-xl font-bold border border-rose-200 dark:border-rose-800">
                🚨
              </div>
              <div>
                <h3 className="text-lg font-bold text-slate-900 dark:text-white">
                  Reportar Incumplimiento de Guardia
                </h3>
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  Aplicación automática de multa de $24.000 (Reglamento Art. 9)
                </p>
              </div>
            </div>
            ...
            <form onSubmit={handleReportarIncumplimiento} className="space-y-4">
              <div>
                ...
                <select
                  ...
                  className="w-full h-10 px-3 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white text-xs focus:outline-none focus:ring-2 focus:ring-rose-500"
                >
                  {turnos.map((t) => (
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full h-10 px-3 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500"
                >
                  {turnos.map((t) => (
                    <option key={t.id} value={t.numero_uf}>
                      UF 0{t.numero_uf} — {t.piso_depto} ({t.residente})
              ...
              <div className="flex gap-3 pt-2">
                ...
                >
                  {reportingLoading ? "Aplicando..." : "Confirmar e Imputar Multa"}
                </button>
              </div>
            </form>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 backdrop-blur-sm p-4 animate-fade-in">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl max-w-md w-full p-6 shadow-2xl relative">
            ...
            <div className="text-center mb-5">
              <div className="w-12 h-12 rounded-xl bg-indigo-50 dark:bg-indigo-950 text-indigo-600 dark:text-indigo-400 flex items-center justify-center text-2xl mx-auto mb-2">
                ⇄
              </div>
              <h3 className="text-lg font-bold text-slate-900 dark:text-white">
              ...
              {turnos
                ...
                .map((t) => (
                  ...
                  >
                    ...
                    <span className="text-xs font-semibold text-indigo-600 dark:text-indigo-400 opacity-0 group-hover:opacity-100 transition">
                      Intercambiar →
                    </span>
                  </button>
                ))}
            ...
            <button
              onClick={() => setShowPermutaModal(false)}
              className="w-full py-2.5 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 font-medium rounded-xl text-xs hover:bg-slate-100 dark:hover:bg-slate-800 transition"
            >
              Cancelar
            </button>
          </div>
