  ...
  try {
    ...
    for (const exp of loteExpensas) {
      await prisma.expensa.upsert({
        ...
        update: { montoOrdinario: exp.monto_ordinario, montoExtraordinario: exp.monto_extraordinario, montoCargos: exp.monto_cargos, recargoMora: exp.recargo_mora, totalPagar: exp.total_pagar, fechaVencimiento: new Date(exp.fecha_vencimiento) },
